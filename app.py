...
@app.route('/')
def Index():
    str = 'This API have the following routes: <br>' \
        + '/api/v1.0/precipitation -- Returns precepitation data of the entire range <br>'\
        + '/api/v1.0/stations -- Returns all the stations <br>'  \
        + '/api/v1.0/tobs -- Temperature Observation of the most active station <br>' \
        + '/api/v1.0/<start> and /api/v1.0/<start>/<end> to get the observations tmin, tavg, tmax of the given range or all dates after a start date.'
...
def tobs():
    ...
    top_active_station = session.query(func.min(Measurement.tobs), func.max(Measurement.tobs), func.avg (Measurement.tobs)).\
